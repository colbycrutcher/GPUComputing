...
unsigned int cpuReduce(float *h_in, int n)
{
    unsigned int s = 0;
    int i;
    for (i = 0; i < n; i++)
    {
        s += (unsigned int)h_in[i];
    }
    return s;
}
...
__global__ void reduce3(float *in, float *out, int n)
{
    ...
    unsigned int tid = threadIdx.x;
    // loads two elements into shared memory
    unsigned int i = blockIdx.x * (blockDim.x * 2) + threadIdx.x;

    // Load from global memory to shared memory
    sdata[tid] = (i < n) ? in[i] : 0;
    if (i + blockDim.x < n)
        sdata[tid] += in[i + blockDim.x];

    __syncthreads();

    // does reduction in shared memory using Sequential Addressing
    // stride = half the block size, then half it each step
    ...
    //  result for this block to global memory
    if (tid == 0)
        out[blockIdx.x] = sdata[0];
}
...
void runCPU(float *arr, int n)
{
    ...
    // if (shouldPrint)
    printf("CPU finding sum number is %d\n", sum);
}
