...
The device's global memory is persistent across kernel launches, meaning it isn't wiped when one kernel is finished, and another begins. Data written by the first kernel remains available to the second kernel if the execution order is proper by using something like cudaDeviceSynchronize() to ensure all the threads are up to date before moving on. Therefore, data can be passed between kernels directly through global memory without reallocating or copying, but the programmer is responsible for proper synchronization and initialization.